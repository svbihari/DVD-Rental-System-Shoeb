...
public class SystemFile {
	...
	public static void main(String[] args) {
		...
		// Read current DVD input File
		Functions.readCurrentFile();
		...
		while (!exit) {
			...
			// Condition if user inputs login and there is no current session it
			// logs in
			...
			// Condition for other input test, Does not permit following
			// transaction prior to login

			else if (input.equalsIgnoreCase("rent")
					|| input.equalsIgnoreCase("sell")
					|| input.equalsIgnoreCase("buy")
					|| input.equalsIgnoreCase("remove")
			...
			// Condition to terminate system
				...
				// Exit takes place only if login is set to true meaning user
				// must be logged in

				if (login) {
					System.out
							.println("Error: You must log out before exiting system");
				}
